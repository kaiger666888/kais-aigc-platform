...
- [x] **VERIFY-02**: ACE-Step 容器健康（`Status: healthy`），日志中无 `PermissionError` — 关闭 v1.3 FIX-02 *(✅ Worked around via localhost subprocess mode + transformers 4.57.1 + checkpoints symlink. ACE-Step API responds `models_initialized:true`, engine registered as `acestep-internal` with backend_type=docker. Standalone container blocked on registry mirror — see commit ccf78a0 for details.)*
- [ ] **VERIFY-03**: 通过 `POST http://localhost:8002/api/v1/tasks {"task_type":"music", ...}` 触发 ACE-Step 生成可播放的 MP3 文件（非 mock 输出），端到端跑通 — 关闭 v1.3 FIX-03 *(⚠️ HARDWARE-BLOCKED: Task routes correctly to ACE-Step, but inference OOMs — ACE-Step XL DiT + 4B LM = 23.5GB resident on the 24GB RTX 3090 leaves <400MB for inference working set. Localhost subprocess mode shares GPU 1 with comfyui-primary; original design assumed dedicated container with exclusive GPU. Needs: >24GB GPU, OR mirror restoration so standalone kais-acestep can run with exclusive GPU, OR ACE-Step offload tuning.)*
