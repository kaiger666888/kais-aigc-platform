...
- [x] **HIER-01**: 资产管理中心三层资产层级视图——阶段/资产类型 → 候选组(variantGroupId 语义) → 候选;层间折叠导航 + 每层选定/待选/淘汰计数聚合;单产物(无组)资产有明确挂载位
- [x] **HIER-02**: 选定逻辑层级化——组内 winner 走既有 select-winner 事务闭环;层级聚合批量决策入口(形态 discuss 定夺);DAG `has-candidates` 计数与层级视图一致
- [x] **HIER-03**: 画布内 pre/final 冗余配置入口——读侧展示各资产类型 n_candidates/final_candidates 当前值,写侧可编辑;写入通道契约 discuss 定夺并成文;键面覆盖 kmc v2.5 全域矩阵,不可配键显式标注原因
- [x] **HIER-04**: 三态流转零回归——分组互斥/取消选定恢复/场景声纹手动规则/G13 首尾分选在层级视图下语义保持(负向断言锁死)
- [x] **HIER-05**: e2e 三链路(层级导航/层级化选定/冗余配置读写)+ 既有资产管理 e2e 全量零回归
...
| HIER-01 | Phase 62 | Complete |
| HIER-02 | Phase 62 | Complete |
| HIER-03 | Phase 62 | Complete |
| HIER-04 | Phase 62 | Complete |
| HIER-05 | Phase 62 | Complete |
