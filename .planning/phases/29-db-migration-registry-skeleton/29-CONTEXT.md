...
- **`o_assets.skill_id`** and **`kv_pipelineRun.skill_id`** are added as **nullable TEXT columns with no default**. The Phase 29 backfill populates existing rows with `movie-v1`. New rows are written by Phase 31's refactored callbacks (the writer is responsible for setting skill_id explicitly).
