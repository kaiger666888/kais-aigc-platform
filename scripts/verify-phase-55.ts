#!/usr/bin/env tsx
/**
 * verify-phase-55.ts — Phase 55 (NAV-01) 契约测试:kap PHASE_REGISTRY(22 phase
 * 单一注册表)≡ khs 三真相源,任一漂移即红(D-01 镜像纪律,verify-schema-drift
 * 第三次复刻)。
 *
 ...
 *   - skills/kais-movie-pipeline/pipeline/phases/__init__.py PHASE_REGISTRY (22 活跃 id)
 *
 * 断言组:
...
function main(): void {
  ...
  assert(khsIds.length === 22, `S-parse: khs PHASE_REGISTRY 22 活跃 id`, String(khsIds.length));
  ...
  assert(PHASE_REGISTRY.length === 22, `A: kap 注册表恰 22 条`, String(PHASE_REGISTRY.length));
