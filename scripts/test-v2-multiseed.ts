...
async function poll(promptId: string, timeoutMs = 600_000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const r = await axios.get(`${COMFYUI_URL}/history/${promptId}`, { timeout: 10_000 });
    const e = r.data?.[promptId];
    if (e?.status?.completed) return { ok: true, outputs: e.outputs };
    if (e?.status?.status_str === "error") return { ok: false, error: "exec error" };
...
async function main() {
  ...
  for (const p of REF_IMAGES) {
    const fn = `multiseed_${uuidv4()}${path.extname(p)}`;
    execSync(`docker cp "${p}" ${CONTAINER}:"${INPUT_DIR}/${fn}"`, { timeout: 30_000 });
    refFilenames.push(fn);
  }

  const SEEDS = [200];
  const NUM_FRAMES = roundTo8nPlus1(73);
  const results: any[] = [];
  for (const seed of SEEDS) {
    console.log(`\n==> seed=${seed}`);
    const wf = buildWorkflow({ refFilenames, audioGuide: AUDIO_V2, numFrames: NUM_FRAMES, msrFrameCount: 41, fps: 24, seed, filenamePrefix: `ltx_audio_v2_bgmseed/seed${seed}` });
    const r = await axios.post(`${COMFYUI_URL}/prompt`, { prompt: wf }, { timeout: 30_000, validateStatus: s => s < 500 });
    if (r.status !== 200) { console.error("submit failed:", JSON.stringify(r.data).slice(0, 300)); continue; }
    const pid = r.data.prompt_id;
    const result = await poll(pid);
    if (!result.ok) { console.error("poll failed:", result.error); continue; }
    const v = result.outputs?.["20"]?.videos?.[0] || result.outputs?.["20"]?.images?.[0];
    console.log(`  ✓ done: ${v?.filename}`);
    results.push({ seed, promptId: pid, ...v });
  }
  fs.writeFileSync("/tmp/ltx-audio-v2-test/multiseed.json", JSON.stringify(results, null, 2));
  console.log("\n==> written /tmp/ltx-audio-v2-test/multiseed.json");
}
