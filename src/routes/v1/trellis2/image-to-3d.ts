...
export default router.post("/", upload.single("image"), async (req, res) => {
  ...
  // TRELLIS2 3D 重建 (ENGINE_VRAM_REQUIREMENTS.trellis2) 与 TTS/H3/music3/qwen_eye
  // 共享 GPU1 锁 — 此前直提 ComfyUI 绕过队列, 是同卡撞车源。异步 taskId 模式:
  // 锁只包「提交段」(显存预检/驱逐 + POST /prompt), 作业在 ComfyUI 侧异步跑,
  // 客户端轮询 status 路由。multipart 解析/容器拷贝/失败清理均在队列外。
  try {
    const submitted = await withGpuQueue(
      ...
      { gpuIndex: 1, comfyuiUrl: TRELLIS2_CONFIG.comfyuiUrl },
    );
