  ...
  return {
    ...
    "34": {
      ...
      inputs: {
        ...
        guider: ["33", 0],
        sampler: ["30", 0],
        sigmas: useInterp ? [H3_SIGMA_INTERP_NODES.ref2va, 0] : ["31", 0],
    ...
    // === 音频解码 (合并到视频) ===
    "43": { class_type: "VAEDecodeAudio", inputs: { samples: ["34", 0], vae: ["13", 0] } },
...
export default router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // ref2va 与 /generate 同引擎 (minimax_h3) 必须同锁 — 此前直提 ComfyUI 绕过
      // 队列, 是同卡撞车源。异步 taskId 模式: 锁只包「提交段」(显存预检/驱逐 +
      // POST /prompt), 作业在 ComfyUI 侧异步跑, 客户端轮询 /status。
      // 参考图/音/视频等 multipart 解析与容器拷贝均在队列外 (不持锁等上传)。
