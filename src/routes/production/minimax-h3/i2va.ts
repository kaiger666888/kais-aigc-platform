...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (tokenBudget.level === "warn") {
      console.warn(
        `[i2va] H3 token budget WARN: ${width}×${height}×${length}f = ` +
    ...
    try {
      ...
      // i2va/fl2va/l2va 与 /generate 同引擎 (minimax_h3) 必须同锁 — 此前直提
      // ComfyUI 绕过队列, 是同卡撞车源。异步 taskId 模式: 锁只包「提交段」
      // (显存预检/驱逐 + POST /prompt), 作业在 ComfyUI 侧异步跑, 客户端轮询 /status。
      // multipart 解析/容器拷贝均在队列外 (不持锁等上传)。
