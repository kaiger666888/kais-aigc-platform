...
export function buildH3T2vaWorkflowNative(opts: H3T2vaWorkflowOpts): Record<string, any> {
  ...
  return {
    ...
    // 仅当 H3_TESPEED.enabled=true 且 tespeed !== false 时插入 TESpeed (native-sage=false 不插入)。
    ...(useTespeed ? {
      [H3_TESPEED.nodeId]: {
    ...
    "34": {
      ...
      inputs: {
        ...
        guider: ["33", 0],
        sampler: ["30", 0],
        sigmas: useInterp ? [H3_SIGMA_INTERP_NODES.t2va, 0] : ["31", 0],
        latent_image: ["20", 1],
      },
    },
...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (tokenBudget.level === "warn") {
      console.warn(
        `[t2va] H3 token budget WARN: ${width}×${height}×${length}f = ` +
