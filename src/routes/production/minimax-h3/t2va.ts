 ...
 *   turbo          : boolean (true=启用 Turbo LoRA, 4 步加速;默认 false)
...
interface H3T2vaWorkflowOpts {
  ...
  steps: number;         // turbo 时由 builder 覆盖为 H3_TURBO.turboSteps
  shiftVideo: number;
  shiftAudio: number;
  refImageSize: "match" | "max";
  filenamePrefix: string;
  turbo?: boolean;       // true=插入 LoraLoaderBypassModelOnly, 4 步加速
}
...
export default router.post(
  ...
  async (req, res) => {
    ...
    // negativePrompt 在 T8 下不再使用 (无负面条件节点); 接受该字段以保持 API 向后兼容。
    ...
    try {
      ...
      res.status(200).send(success({
        ...
        message: "H3 t2va task submitted to ComfyUI (T8 Dual-Clock)",
      }));
    } catch (err: any) {
