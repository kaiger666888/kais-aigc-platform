...
import { buildFFLFWorkflow } from "./_shared/workflows";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const filenamePrefix = req.body.filenamePrefix || `wan22_fflf_${projectId}_${Date.now()}`;
    ...
    if (!files?.firstFrame?.[0] || !files?.lastFrame?.[0]) {
      return res.status(400).send(error("firstFrame and lastFrame are required"));
    }
    ...
    try {
      copyToContainer(firstFile.path, `${WAN22_CONFIG.comfyuiInputDir}/${firstFrameFilename}`);
      copyToContainer(lastFile.path, `${WAN22_CONFIG.comfyuiInputDir}/${lastFrameFilename}`);
    } catch (err: any) {
    ...
    try {
      ...
      res.status(200).send(success({ promptId, status: "pending", workflowType: "fflf", message: "Wan 2.2 FFLF submitted", firstFrameFilename, lastFrameFilename }));
    } catch (err: any) {
