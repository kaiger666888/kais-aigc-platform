...
import { buildMoveTrackWorkflow } from "./_shared/workflows";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const filenamePrefix = req.body.filenamePrefix || `wan22_movetrack_${projectId}_${Date.now()}`;
    ...
    const workflow = buildMoveTrackWorkflow({
      inputFilename, prompt, negativePrompt,
      width, height, numFrames, fps, seed,
      stepsStage1, stepsStage2, shift, samplerName, scheduler, strength,
    ...
    try {
      ...
      res.status(200).send(success({ promptId, status: "pending", workflowType: "movetrack", message: "Wan 2.2 MoveTrack submitted", inputFilename }));
    } catch (err: any) {
