...
/** 读取某节点的 meta（V3 storyboard 元数据权威存放处，canonical 回写后经 graphToViewModel 注入 data.meta）。 */
...
test.describe('Phase 35 — Storyboard Metadata', () => {
  ...
  test('STORYBOARD-06: dropdown change updates store immediately', async ({ page }) => {
    ...
    // V3：卡片不再渲染 chip；改验证 canonical meta 已即时更新为 wide（→ 远景）
    await expect.poll(async () => (await getMeta(page, 'storyboard-1')).framing, { timeout: 3_000 }).toBe('wide')
  })
  ...
  test('STORYBOARD-06: "未设置" option clears the field', async ({ page }) => {
    ...
    await expect.poll(async () => (await getMeta(page, 'storyboard-1')).cameraMovement, { timeout: 3_000 }).toBe('zoom_in')
    await select.selectOption('')
    await expect.poll(async () => (await getMeta(page, 'storyboard-1')).cameraMovement, { timeout: 3_000 }).toBeUndefined()
  })
  ...
  test('STORYBOARD-07: save → reload round-trip preserves metadata', async ({ page }) => {
    ...
    // 验证后端收到的保存是 V2 wire 形状（save-v2 契约）：meta.version==='2'、节点带
    // branchId；storyboard meta 被序列化器摊平进 data（reload 时 buildMeta 只读 flat 字段）
    const saved = await page.evaluate(async () => {
      const res = await fetch('/__mock/state')
      return res.json()
    })
    expect(saved.canvas.meta?.version).toBe('2')
    const sb1Node = saved.canvas.nodes.find((n) => n.id === 'storyboard-1')
    expect(sb1Node.branchId).toBeTruthy()
    expect(sb1Node.data.framing).toBe('wide')
    expect(sb1Node.data.cameraMovement).toBe('zoom_in')

    // 真往返：save-v2 广播 graph:saved → 前端自动 reload → canonical meta 回读
    //（编辑经 保存→后端→load-v2→adapter 全链路后仍在，最接近成功标准 3）
    await expect.poll(async () => (await getMeta(page, 'storyboard-1')).framing, { timeout: 5_000 }).toBe('wide')
    await expect.poll(async () => (await getMeta(page, 'storyboard-1')).cameraMovement, { timeout: 5_000 }).toBe('zoom_in')
  })
