 ...
 *
 * 选中一个资产节点 → 沿因果边求「祖先链（向后全闭包）+ 直接下游（向前一跳）」，
 * 其余节点压暗 0.28 + desaturate。sequence 邻居不算祖先（P11：排序非因果）；
 * isInactive 边（非优胜变体下游）不参与溯源（P12×P18）。
 *
...
export function computeTraceHighlight(graph: FlowGraphV3 | null, selectedId: string | null): TraceResult {
  ...
  // 反向邻接（target → sources）求祖先；正向邻接（source → targets）求下游
  ...
  // 直接下游：正向一跳
  for (const s of succs.get(selectedId) ?? []) highlightedIds.add(s)
