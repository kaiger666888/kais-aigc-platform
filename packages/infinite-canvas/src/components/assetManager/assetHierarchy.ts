 ...
 * restoreAsset），资产库（AssetLibrary）视图调用（08-25 选片决策视图退役后单调用点；
 * 此前两视图同调用点 HIER-04）。handler 经 ctx 注入 assets/patchLocal/reload/
 * showToast 等组件闭包依赖——模块本身不 import store / 不 import React，
 * 但会发起网络副作用（updateAsset / selectVariantWinner），故属 handler 家而非纯函数家。
 *
 * 62-05 批量决策层（planBatch 与 runBatch 两族）随选片决策视图 08-25 退役整体移除；
 * assetFreshnessKey/pickLatestActive 保留——自动初始化 winner 规则仍消费。
 *
...
// ─── 「最新非淘汰」winner 规则（62-05；批量层已退役，自动初始化仍消费） ──────────

/**
 * 「最新」排序键（winner 规则）：updatedAt ?? createdAt ?? id 三段键。
...
/**
 * 组内「最新非淘汰」候选（自动初始化 winner 规则；62-05 与批量选定同规则
