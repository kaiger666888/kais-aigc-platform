...
## 9. QC 卡服务级负载压测（✅ 2026-09-05/06 Hermes 实操，与 §8 双向验证互证）

| 项 | 结果 |
|---|---|
| qwen-ear 实转落卡 | 角色链 UUID 形态注入 CUDA_VISIBLE_DEVICES，21.9GB / 1995MHz，:8126 健康（页缓存预热后热启 5s，冷读 ~2min）|
| 真实负载 | Case08 双视频音频 8 个互异变体连续转写，峰值 367W / 时钟满格无降频，与 §8 的 369W 双向验证一致 |
| 温控联动实战 | pwm 64→112→160→171 随温自动爬坡，满载温度稳态 ≤50°C 平台 |
| 方法论坑 | 同一音频重复喂被 KV 前缀缓存吃掉（cached_tokens 1188/1189，0.1s 假负载压不出热）→ 压测必须用互异输入 |
| 还卡 | QC 卡 279MiB 归还，渲染卡全程无扰动 |
