...
from transformers import CLIPTextModel, CLIPTextConfig, T5Config, T5EncoderModel
...
def load_clip_l(
    ...
    config = CLIPTextConfig(**CLIPL_CONFIG)
    ...
    if state_dict is not None:
        ...
        logger.info(f"Loading state dict from {ckpt_path}")
        sd = load_safetensors(ckpt_path, device=str(device), disable_mmap=disable_mmap, dtype=dtype)
    # strip text_model. prefix for CLIPTextModel compatibility
    renamed_sd = {k.replace('text_model.', '', 1) if k.startswith('text_model.') else k: v for k, v in sd.items()}
    info = clip.load_state_dict(renamed_sd, strict=False)
